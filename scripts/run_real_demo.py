...
def main() -> None:
    ...
    if actual < request.min_duration_seconds:
        raise RuntimeError(f"Russian speech is only {actual:.2f}s; expected at least {request.min_duration_seconds}s")

    manifest = {"topic":args.topic,"duration_seconds":request.duration_seconds,"fps":request.fps,"width":request.width,"height":request.height,"assets":assets,"audio":"voice_ru.mp3","narration":NARRATION}
    (public/"manifest.json").write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding="utf-8")

    subprocess.run(
        ["npm","run","render","--","RealVertical",str(root/"output"/"real_demo.mp4"),"--props",json.dumps(manifest,ensure_ascii=False)],
